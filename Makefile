...
M     := main
C     := common
FILES := $(C)/Orcish $(C)/BoundedReader $(C)/TextReader $(C)/BitVector $(C)/UnrecognisedTokenException $(M)/FourOneFourMud $(M)/Tests $(M)/Loader $(M)/Area $(M)/Connection $(M)/Commandset $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Money $(E)/Room
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
