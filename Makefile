...
M     := main
C     := common
FILES := $(C)/Orcish $(C)/BoundedReader $(C)/TextReader $(C)/BitVector $(C)/Chance $(C)/UnrecognisedTokenException $(M)/Mud $(M)/Hit $(M)/Area $(M)/Connection $(M)/Chronos $(M)/Command $(M)/Mapper $(M)/Hit $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Equipment $(E)/Money $(E)/Room
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
EXTRA := go.sh todo.txt data/* data/areas/*.area data/commandsets/*.cset data/damagetypes/*.damt
INST  := $(PROJ)-$(VA)_$(VB)
