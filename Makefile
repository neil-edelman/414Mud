...
M     := main
C     := common
FILES := $(C)/Orcish $(C)/BoundedReader $(C)/TextReader $(C)/BitVector $(C)/Buffer $(C)/Chance $(C)/UnrecognisedTokenException $(M)/FourOneFourMud $(M)/TestLoader $(M)/Hit $(M)/Loader $(M)/Area $(M)/Connection $(M)/Commandset $(M)/Mapper $(M)/Hit $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Money $(E)/Room
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
EXTRA := go.sh todo.txt $(BDIR)/data/* $(BDIR)/data/areas/*.area $(BDIR)/data/commandsets/*.cset
INST  := $(PROJ)-$(VA)_$(VB)
