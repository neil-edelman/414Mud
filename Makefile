...
E     := entities
M     := main
FILES := $(M)/FourOneFourMud $(M)/Area $(M)/Connection $(M)/Commandset $(M)/Orcish $(M)/BoundedReader $(M)/MoreReader $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Money $(E)/Room
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
