...
M     := main
C     := common
FILES := $(C)/Orcish $(C)/BoundedReader $(C)/TextReader $(C)/BitVector $(C)/Chance $(C)/UnrecognisedTokenException $(M)/Mud $(M)/Hit $(M)/Area $(M)/Connection $(M)/Chronos $(M)/Command $(M)/Mapper $(M)/Hit $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Money $(E)/Room $(E)/PlayerLike
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
