...
M     := main
C     := common
FILES := $(C)/Orcish $(C)/BoundedReader $(C)/TextReader $(C)/BitVector $(C)/Chance $(C)/UnrecognisedTokenException $(M)/Mud $(M)/Area $(M)/Connection $(M)/Chronos $(M)/Command $(M)/Mapper $(E)/Stuff $(E)/Character $(E)/Player $(E)/Mob $(E)/Object $(E)/Container $(E)/Equipment $(E)/Money $(E)/Room
SDIR  := src
BDIR  := bin
BACK  := backup
ICON  :=
